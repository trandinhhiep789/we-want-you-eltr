...
router.put("/tuyendung/update_user", function (req, res, next) {
    ...
    if(req.body.imageUrl && req.body.imageUrl.length > 0){
        newValues.imageUrl = req.body.imageUrl;
    }

    // Update Images Cover
    if(req.body.imageUrlCover && req.body.imageUrlCover.length > 0){
        newValues.imageUrlCover = req.body.imageUrlCover;
    }
    ...
    User.findOneAndUpdate(
      condition,
      { $set: newValues, $addToSet:{kinhNghiemLamViec: req.body.kinhNghiemLamViec} },
      options,
      (err, updateUser) => {
        if (err) {
          res.json({
